...
class Network(object):
    ...
    def conv(self, input, k_h, k_w, c_o, s_h, s_w, name, relu=True, padding=DEFAULT_PADDING, group=1, trainable=True):
        ...
        with tf.variable_scope(name) as scope:
            ...
            kernel = self.make_var(
                'weights', [k_h, k_w, c_i / group, c_o], init_weights, trainable)
    ...
    def proposal_target_layer(self, input, classes, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            labels = tf.convert_to_tensor(
                tf.cast(labels, tf.int32), name='labels')
    ...
    def fc(self, input, num_out, name, relu=True, trainable=True):
        with tf.variable_scope(name) as scope:
            ...
            weights = self.make_var(
                'weights', [dim, num_out], init_weights, trainable)
    ...
    def _activation_summary(x):
        """Helper to create summaries for activations.Creates a summary that provides a histogram of activations. Creates a summary that measures the sparsity of activations.
        Args:
        x: Tensor
        Returns:
        nothing
